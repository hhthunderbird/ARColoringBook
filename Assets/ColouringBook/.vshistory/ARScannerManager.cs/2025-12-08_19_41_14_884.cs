...
namespace Felina.ARColoringBook
{
    public class ARScannerManager : MonoBehaviour
    {
        ...
        [Range( 0f, 1f )]
        [SerializeField] private float captureThreshold = 0.85f;
    ...
    [System.Runtime.InteropServices.DllImport("Felina")]
        ...
        void Start()
        {
            ...
            _unwarpedTexture = new RenderTexture( _outputResolution, _outputResolution, 0, format )
            {
                useMipMap = false,
                autoGenerateMips = false,
                filterMode = FilterMode.Bilinear
            ...
            while ( ( !_isLocked ) && image != null && !token.IsCancellationRequested )
            {
                if ( image.trackingState == TrackingState.Tracking )
                {
                    ...
                    if ( IsDeviceStable )
                    {
                        ...
                        if ( _autoLock && currentScore >= captureThreshold )
                        {
                            _isLocked = true;
