...
namespace Felina.ARColoringBook
{
    public class ARScannerManager : MonoBehaviour
    {
        ...
        public RenderTexture unwarpedTexture;
    ...
    [System.Runtime.InteropServices.DllImport("Felina")]
        ...
        void Start()
        {
            ...
            if ( !_imageManager ) _imageManager = FindAnyObjectByType<ARTrackedImageManager>();
            ...
            if ( unwarpedTexture != null ) unwarpedTexture.Release();
            unwarpedTexture = new RenderTexture( outputResolution, outputResolution, 0, format )
            {
                useMipMap = false,
                autoGenerateMips = false,
                filterMode = FilterMode.Bilinear
            };
            unwarpedTexture.Create();
            ...
            if ( arCamera != null )
            {
                _lastCamPos = arCamera.transform.position;
                _lastCamRot = arCamera.transform.rotation;
            }
        }
        ...
        void OnEnable() { if ( _imageManager ) _imageManager.trackablesChanged.AddListener( OnTrackablesChanged ); }
        void OnDisable() { if ( _imageManager ) _imageManager.trackablesChanged.RemoveListener( OnTrackablesChanged ); }
        ...
        private void OnTrackablesChanged( ARTrackablesChangedEventArgs<ARTrackedImage> param )
        {
            if ( _isLocked ) return;
        ...
        private async UniTaskVoid TrackAndPaint( ARTrackedImage image, CancellationToken token )
        {
            // Simple loop while tracking
            while ( ( !_isLocked ) && image != null && !token.IsCancellationRequested )
            {
                if ( image.trackingState == TrackingState.Tracking )
                {
                    ...
                    if ( IsDeviceStable )
                    {
                        float currentScore = CalculateQualityScore( image );

                        // Debug Log to see why it fails
                        // Debug.Log($"[Quality] Score: {currentScore} (Best: {_highestQualityScore})");

                        if ( currentScore > _highestQualityScore )
                        {
                            _highestQualityScore = currentScore;

                            EnsureCameraFeedIsFresh();
                            ProcessImage( image );
                        }

                        if ( autoLock && currentScore >= captureThreshold )
                        {
                            _isLocked = true;
        ...
        private void CalculateStability()
        {
            ...
            float3 currentPos = arCamera.transform.position;
        ...
        {
            if ( arCameraBackground == null || arCameraBackground.material == null ) return;
            if ( Time.frameCount != _lastFrameUpdated )
            {
                Graphics.Blit( null, _cameraFeedRT, arCameraBackground.material );
        ...
        private float CalculateQualityScore( ARTrackedImage image )
        {
            // 1. Gather Data
            float3 camPos = arCamera.transform.position;
            float3 camFwd = arCamera.transform.forward;
            float3 imgPos = image.transform.position;
            float3 imgUp = image.transform.up;

            // 2. Screen Position (Still needs Unity Camera to calculate)
            float3 screenPos3 = arCamera.WorldToScreenPoint( imgPos );
        ...
        private unsafe void ProcessImage( ARTrackedImage image )
        {
            if ( unwarpMaterial == null || unwarpedTexture == null || arCamera == null ) return;

            // 1. Get Physical Size (No Aspect Ratio calculation needed here anymore!)
            float imgW = image.size.x;
            float imgH = image.size.y;
            ...
            unwarpMaterial.SetMatrix( "_DisplayMatrix", float4x4.identity );

            Graphics.Blit( null, unwarpedTexture, unwarpMaterial );

            if ( _targetObjectMaterial != null )
            {
                _targetObjectMaterial.SetTexture( "_BaseMap", unwarpedTexture );
                _targetObjectMaterial.SetTexture( "_MainTex", unwarpedTexture );
                _targetObjectMaterial.SetTexture( "_DrawingTex", unwarpedTexture );
            }
        }

        private float2 ScreenPoint( float3 worldPos )
        {
            float3 s = arCamera.WorldToScreenPoint( worldPos );
            return new float2( s.x, s.y );
        }

        private float2 WorldToNormScreen( float3 worldPos, float invW, float invH )
        {
            float3 s = arCamera.WorldToScreenPoint( worldPos );
            return new float2( s.x * invW, s.y * invH );
        }
