...
namespace Felina.ARColoringBook
{
    ...
    [System.Runtime.InteropServices.DllImport("Felina")]
        ...
        void Start()
        {
            ...
            _unwarpedTexture = new RenderTexture( _outputResolution, _outputResolution, 0, _arBridge.RenderTextureFormat)
            {
                useMipMap = false,
                autoGenerateMips = false,
                filterMode = FilterMode.Bilinear
                ...
                if ( target.IsTracking )
                {
                    ...
                    if ( IsDeviceStable )
                    {
                        ...
                        if ( target.Score > _highestQualityScore )
                        {
                            ...
                            _arBridge.FillCameraTexture( _arBridge.GetCameraFeedRT() );
